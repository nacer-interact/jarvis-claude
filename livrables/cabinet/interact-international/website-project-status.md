...
- **Fixed white margins beside the photo bands**: on wide viewports, the area to the left/right of each centered 1140px photo was white, not ivory. Root cause: `.photoband` had no `alignfull` class, so WordPress's own layout CSS capped it at 720px (the default constrained content-size) regardless of any background-color set on it — the ivory fill only ever covered that narrow 720px box, not the viewport-wide strip actually visible behind the centered image. Restructured to `<section class="photoband alignfull"><div class="photoband-inner">...</div></section>`: the outer section now bleeds full width in ivory (matching the other bands), while the new inner wrapper keeps the image itself centered and capped at 1140px, exactly as before. Bumped the child theme version again (1.5.0 → 1.6.0). Also brought the prototype in line with this same structure and made ivory its default content background (was defaulting to white with ivory as the toggled "alternative"; flipped since ivory is now the shipped design). Verified via computed styles (not just eyeballing) that `.photoband` spans the full viewport while `.photoband-inner` stays at 1140px, on Home, Services, and Sectors & Approach
...
### 2026-08-19
- **Ran the Rank Math setup wizard** (Easy mode). Skipped the "Connect FREE Account" step (would require logging into a Rank Math/rankmath.com account on Nacer's behalf) and the "Analytics" step's Google Search Console/Analytics connection (would require his Google login) — both are account-authentication steps I shouldn't perform myself; flagged as open items for Nacer instead
- On the "Your Site" step, corrected two fields that were still carrying the stale pre-rename value: Website Name and Person/Organization Name both showed "Interact International Corporate portal" (the old site title, before it was simplified) instead of "Interact International". Also set the site type to "Small Business Site" (was defaulting to "Personal Blog") and uploaded the square logo mark (reused from the favicon work) as the "Logo for Google" image
- Easy mode skips the Sitemaps/Optimization wizard steps entirely and applies sensible defaults directly — confirmed Rank Math's own XML sitemap (`sitemap_index.xml`) is now live and correctly listing post/page/category sub-sitemaps, replacing WP core's fallback sitemap
